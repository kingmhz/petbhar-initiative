...
const PRESET_AMOUNTS = [
  ...
  { amount: 1200, label: '1 Ration Kit' },
  { amount: 2500, label: '50 Meals (Mini Drive)' },
  { amount: 5000, label: '100 Meals + Banner' },
  { amount: 10000, label: '250 Meals Drive' },
];
...
export default function ImpactSimulator({ onOpenDedication, onOpenQR }: ImpactSimulatorProps) {
  ...
  const calculated = useMemo(() => {
    const meals = Math.max(1, Math.floor(amount / 50));
    const bowls = Math.max(2, Math.floor(amount / 25));
    const kits = Math.floor(amount / 1200);
    const attaKg = (amount * 0.035).toFixed(1);
    const riceKg = (amount * 0.025).toFixed(1);
    const dalKg = (amount * 0.012).toFixed(1);
    const oilLitre = (amount * 0.005).toFixed(1);
    ...
    <div className="w-full bg-linear-to-br from-cream/40 to-warm-ivory/20 rounded-3xl border border-charcoal/10 p-6 sm:p-8 shadow-xs">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 mb-6">
        ...
        <div className="p-4 rounded-2xl bg-white border border-charcoal/10 flex items-start gap-3 shadow-xs">
          ...
          <div>
            <div className="font-serif text-2xl font-bold text-charcoal leading-none">
              {calculated.kits > 0 ? calculated.kits : '< 1'}
            </div>
            <span className="text-xs font-semibold text-charcoal block mt-1">
              {t('sim_ration_kits')}
            </span>
            <span className="text-[11px] text-warm-grey">
              Month of staples for 1 family
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl bg-warm-ivory/30 border border-charcoal/10 flex items-center gap-3 mb-6">
        <div className="w-8 h-8 rounded-lg bg-charcoal/5 text-charcoal flex items-center justify-center shrink-0">
          <ShoppingBag size={16} />
        </div>
        <div className="text-xs text-charcoal flex-1">
